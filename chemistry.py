def main():
    ...
    number_of_moles = mass / total_molar_mass

    # Print the molar mass.
    print(f"\nTotal molar mass: {total_molar_mass:.5f} grams/mole")

    # Print the number of moles.
    print(f"Number of moles: {number_of_moles:.5f} moles")

    # Formula names dictionary
    ...
    # Print formula name from the known_molecules_dict
...
def compute_molar_mass(symbol_quantity_list, periodic_table_dict):
    ...
    for get_symbol_quantity in symbol_quantity_list:

        # Separate the list into symbol and quantity.
        symbol = get_symbol_quantity[SYMBOL_INDEX]
        quantity = get_symbol_quantity[QUANTITY_INDEX]

        # Get the atomic mass for the symbol from the dictionary.
        get_name_and_atomic_mass = periodic_table_dict[symbol]

        atomic_mass = get_name_and_atomic_mass[ATOMIC_MASS_INDEX]
...
# Function to find formula in the dictionary and print formula name
def get_formula_name(formula, known_molecules_dict):
    ...
    if formula in known_molecules_dict:
        ...
        print(f"Formula name: {formula_name.capitalize()}")
